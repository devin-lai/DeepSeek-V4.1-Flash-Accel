...
与此前优化预设在同一台 8× RTX 5090 D 服务器上对比，每项运行三轮，
保留原模型权重与量化格式：

| 负载 | 此前优化预设 | 新预设 | 提升 |
| --- | ---: | ---: | ---: |
| 交互生成，单请求 | 69.86 token/s | **74.75** · fast | **7.0%** |
| 交互生成，8 并发 | 153.05 token/s | **168.49** · fast | **10.1%** |
| 1K 输入 / 128 输出，64 并发 | 176.96 输出 token/s | **217.55** · batch | **22.9%** |
| 8K 预填充，2 并发 | 3,004.34 总 token/s | **4,077.99** · batch | **35.7%** |

- `v41-flash-fast`：适合短交互请求。减少 CPU 专家卸载，固定每卡 256 MiB KV 缓存，
  最多 8 个活跃序列；单请求首 token 延迟增加约 43 ms，但总体输出更快。
- `v41-flash-balanced`：原 16 序列 DSpark 预设加精确权重暂存，适合较长提示词。
  相同配置下，8K 预填充吞吐提高 36.5%，解码测试范围重叠。
- `v41-flash-batch`：64 个活跃序列加精确暂存，适合总吞吐优先的任务。
  64 并发时逐 token 延迟增加 41%，p99 完成延迟增加 16%，需要按负载取舍。

fast 和 batch 均在固定 GSM8K 子集上保持 **126/128**，并通过单个 32K 和
8 个并发 8K 缓存探针。固定输入的内核输出通过逐位一致性检查；这些有限测试
不能证明完整模型质量等价或长期稳定性。
[完整结果、原始数据与限制](benchmarks/results/2026-09-15-staging-and-batching.md)。
...
| 文本 | `v41-flash-fast` 用于短交互，`v41-flash-balanced` 用于长提示词，`v41-flash-batch` 用于批量任务；安装插件 0.3.0 与仓库补丁 |
...
并发批量任务可选 `PRESET=v41-flash-batch`：总吞吐更高，但每个活跃请求的
